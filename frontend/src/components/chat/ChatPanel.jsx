 ...
 * @param {{ chat: { messages: Array, isStreaming: boolean, sendMessage: (value: string) => Promise<void>, clearHistory: () => Promise<void> }, compact?: boolean }} props
 * @returns {JSX.Element}
 */
export default function ChatPanel({ chat, compact = false }) {
  return (
    <div className="flex h-full flex-col rounded-[28px] border border-white/80 bg-white p-5 shadow-panel">
      <div className="mb-4 flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500">Streaming answers with cited references.</p>
        </div>
        <button
          type="button"
      ...
      <div className={`space-y-3 overflow-y-auto pr-1 ${compact ? "max-h-[28rem]" : "flex-1"}`}>
        {chat.messages.length ? (
          ...
          <div className="rounded-3xl bg-slate-50 p-4 text-sm text-slate-500">
            Ask about TDS, GST, filing deadlines, or deductions. Uploaded documents will be used when relevant.
          </div>
        )}
        {chat.isStreaming ? (
